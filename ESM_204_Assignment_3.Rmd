...
```{r}
Qtax <- function(p){
  q <- (Dagg$coefficient[1] - mec51) / (m_MPC - Magg)
  return(q)
}
...
CS_agg_new_high <-CS_agg(Ptax) # use the new P value to calculate the new CS
CS_agg_new_high
```
...
```{r}
CS_agg_new_low <- CS_agg_new_high - (MEC * Qtax)
...
```{r}
PS_new_high <- PS_new(Ptax, model_dem_high)

PS_new_low <- PS_new(Ptax, model_dem_low)
...
#### The welfare benefit to Power Supplies is `r PS_total_1`

#(f) Total environmental damage

(mec51 x Qt) x (mec51 x Qtax)

```{r}
Env_damage <- Qtax * mec51
```
...
```{r}
CS_agg_51_high <- CS_agg_new_high - (mec51 * Qtax)

CS_agg_51_high

CS_agg_75_high <- CS_agg_new_high - (mec75 * Qtax)

CS_agg_75_high

CS_agg_100_high <- CS_agg_new_high - (mec100 * Qtax)

CS_agg_100_high

CS_agg_125_high <- CS_agg_new_high - (mec125 * Qtax)

CS_agg_125_high

CS_agg_150_high <- CS_agg_new_high - (mec150 * Qtax)
