...
# Graph the new Demand Curves (with tax) - Dagg_new_d deined later
```{r  echo=FALSE}
#ggplot(Dagg_new_d) +
...
# Cut and Paste for the rest
```

#### Ob=verall Welfare for High Incpome Comsumers:
...
#CS_agg_51_high <- CS_agg_new_high - (mec51 * Qtax(51))

#CS_agg_51_high

#CS_agg_75_high <- CS_agg_new_high - (mec75 * Qtax(75))

#CS_agg_75_high

#CS_agg_100_high <- CS_agg_new_high - (mec100 * Qtax(100))

#CS_agg_100_high

#CS_agg_125_high <- CS_agg_new_high - (mec125 * Qtax(125))

#CS_agg_125_high

#CS_agg_150_high <- CS_agg_new_high - (mec150 * Qtax(150))
...
#CS_agg_51_low <- CS_agg_new_low - (mec51 * Qtax)

#CS_agg_51_low

#CS_agg_75_low <- CS_agg_new_low - (mec75 * Qtax)

#CS_agg_75_low

#CS_agg_100_low <- CS_agg_new_low - (mec100 * Qtax)

#CS_agg_100_low

#CS_agg_125_low <- CS_agg_new_low - (mec125 * Qtax)
#
#CS_agg_125_low

#CS_agg_150_low <- CS_agg_new_low - (mec150 * Qtax)
...
- PS for electricity producers (will graph) DONT WORRY about this
...
must calculate new Dagg and repear the process to find a new Q*

```{r}
# demand_h_pv <- mh + Pint/2

solar_high <- high_dem_new/2

model_dem_solar <-lm(price ~ demand, data = solar_high)

Dagg_solar_lm <- lm(price ~ demand, data = solar_high)

Dagg_solar <-function(p, model){
  q <- (p - model$coefficients[[1]])/model$coefficients[[2]] - mec51
  q <- ifelse(q<0,0,q)
  return(q)}

Dagg_solar(Ptax51, model_dem_solar)
Dagg_solar_d <- tibble(Qagg = Qagg, price = price)

#  make dataframe and model, combine model with model_dem_low for new Dagg

# find new P*_pv - answer
...
# Solar Tax = mec51 * Qtax_pv (Q1 in Git hub file I uploaded) for new Dagg from (a)
