...
# The tax should be where the MEC meets the Q* (socially optimal quantity). In this case, the MEC is a straight, horizontal line, so the t* will simply equal around 2 cents per Kwh electricity. 

# Graph the demand curves and the aggregate in one graph with the MES, MSC, and MPC

## Step 1: graph demand curves
```{r}
ggplot(model_dem_low, aes(price_cents, q_low_kwh))+
  geom_line()

ggplot(model_dem_high, aes(price_cents, q_high_kwh))+
  geom_line()
```

#(a) Overall welfare of “high” income consumers
- CS for high income consumers (will graph)
