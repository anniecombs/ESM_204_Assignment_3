...
CS_agg_new <-CS_agg(11.29)
```

#### The welfare of high income consumers is equal to `CS_agg_new`
...
PS_new_agg <- function(p){
  ps <-PS(p, high_dem_new) + PS(p, low_dem_new)
  return(ps)}
```
...
ggplot() +
  ...
  geom_line(l_pred, mapping = aes(x = demand, y = price, color = "Low income")) +
  geom_line(h_pred, mapping = aes(x = demand, y = price, color = "High income")) +
  geom_line(MPC_d, mapping = aes(x = demand, y = price, color = "MPC")) +
  ...
  geom_line(MPC, mapping = aes(x = demand, y = price + 2, color = "MSC")) +
  labs(x = "Quantity (kWh)",
       y = "Price (cents)",
       color = "Demand Curve") +
  theme_minimal()
...
Code in demand change for each MEC and from there use high CS functions with each demand model to get your answer
