...
Dagg <- lm(price ~ Q, data = Dagg_d)  # Linerar regression for Dagg(P) for Q > 158,329
...
CS_l_sq <- CS(10, model_dem_low)  # comsumer benefit to low income
...
Find Q*
Q* is equal to the MEC from question 1 and therefore is equal to `r mec51`

#(b) The price of electricity
- This is P*
P = b + mQ
Dagg normal = P = b - t* + mQ 

Set this expression equal to the supply curve

Dagg socially optimal = b - t* + mQ = 0.1Q

b is the y intercept of the Dagg function which is 2.897e+01 as previously calculated.

2.897e+01 - t* + m_MPC = 0.1Q

0.1(`r mec51`) - 2.897e+01 + m_MPC = -t*

t* = 2.5216 cents per Kwh
...
## Step 1: graph demand curves *THESE ARE WRONG - I will fix (Annie)*
```{r}
ggplot(df, aes(Qagg, price)) +
  geom_line()

ggplot(model_1_dem_low, aes(q_low_kwh, price_cents))+
  geom_line()

ggplot(model_1_dem_high, aes(q_high_kwh, price_cents))+
  geom_line()
```
